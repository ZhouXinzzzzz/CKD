...
class BaseTrainer(object):
    def __init__(self, experiment_name, distiller, train_loader, val_loader, cfg):
        ...
        self.optimizer, self.scheduler = self.init_optimizer(cfg)
        self.best_acc = -1
    ...
    def init_optimizer(self, cfg):
        if cfg.SOLVER.TYPE == "SGD":
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer = optimizer,
                                                    T_max = cfg.SOLVER.EPOCHS, eta_min=cfg.SOLVER.LR_COSINE_MIN)
    ...
    def log(self, lr, epoch, log_dict):
        ...
        if self.cfg.LOG.WANDB:
            ...
            wandb.log(log_dict)
            # 似乎每call一次wandb.log會計算一次step。所以显示的step是这玩意乘以2的
    ...
    def train_epoch(self, epoch):
        # clip_validate(self.val_loader, self.distiller) #调试用
        # import pdb
        # pdb.set_trace()
        if self.cfg.SOLVER.LR_COSINE == False: #如果是按epoch更新学习率需要在这里开始变
            lr = adjust_learning_rate(epoch, self.cfg, self.optimizer, self.scheduler)
        ...
        for idx, data in enumerate(self.train_loader):
            ...
            # msg = self.train_iter(data, epoch, train_meters, idx) #通过更新频率来修改batchsize需要在此处加参数
        ...
        # cosine
        if self.cfg.SOLVER.LR_COSINE == True: 
            lr = adjust_learning_rate(epoch, self.cfg, self.optimizer, self.scheduler)
