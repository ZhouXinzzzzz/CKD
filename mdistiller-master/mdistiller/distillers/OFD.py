...
class OFD(Distiller):
    ...
    def ofd_loss(self, feature_student, feature_teacher):
        ...
        min_len  = min(len(self.teacher.get_stage_channels()[1:]), len(self.student.get_stage_channels()[1:]))
        if min_len != len(feature_student):
            feature_student = feature_student[-min_len:]
            feature_teacher = feature_teacher[-min_len:]
